...
* **6. Deploy fast, deploy well**
  ...
  * `pack`
    ...
    * Creates a tar gzipped archive of the current directory named
    `alxwebapp.tar.gz` in the local directory.
  * `deploy`
    * Usage: `fabric -H <remote server IP> deploy`
    * Uploads the archive `alxwebapp.tar.gz` to the `/tmp`
    directory of the remote server.
    * Creates the directory `/tmp/alxwebapp` in the remote server.
    * Untars `alxwebapp.tar.gz` in the `/tmp/alxwebapp` directory
    of the remote server.
  * `clean`
    * Deletes the archive `alxwebapp.tar.gz` in the local directory.
